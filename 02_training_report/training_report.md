...
- **주요 결과**:
  ...
  - 특히 **CatBoost**는 원-핫 인코딩과 결합했을 때 정밀도와 재현율의 완벽한 균형을 보이며 가장 높은 F1-Score를 기록하여 최종 모델로 선정되었습니다.
...
- **데이터 처리**: 모든 범주형 변수에 원-핫 인코딩 적용 및 가중치 미적용
- **최적 파라미터**:
...
| **Recall (재현율)** | **0.80** | 실제 이탈자(90명) 중 72명 검출 |
| **Precision (정밀도)** | **0.97** | 이탈 예측 고객 중 실제 이탈 비율 (오탐 단 2건) |
